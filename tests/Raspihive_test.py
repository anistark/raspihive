...
def logstat():
    ...
    p = subprocess.Popen("sudo service hornet status".split(), stdout=subprocess.PIPE, bufsize=1, text=True)
...
def test():
    ...
    p = subprocess.Popen("ping -c 1 stackoverflow.com".split(), stdout=subprocess.PIPE, bufsize=1, text=True)
    while p.poll() is None:
        msg = p.stdout.readline().strip() # read a line from the process output
        if msg:
            print(msg)
